...
function dragMouseDown(ev) {
  ...
  doc.on('mousemove', function (e) {
    if (!drag) {
      var dX = e.clientX - ev.clientX
      dX *= dX
      var dY = e.clientY - ev.clientY
      dY *= dY
    ...
    drag.forEach(function (sd) {
      sd.leDrag.offset({
        left: e.clientX + sd.relOffsX,
        top: e.clientY + sd.relOffsY,
      })
    })
    drag.data.toast.offset({
      left: e.clientX + 0,
      top: e.clientY + 30,
    })
...
function gridListDrag(ev) {
  currentDir ? deDrag.call(this, ev) : preventDragAndDrop(ev)
}

function preventDragAndDrop(ev) {
  ev = ev.originalEvent
  var dt = ev.dataTransfer
  var t = ev.type
  if (t === 'dragenter') {
...
function processRightClick(ev) {
  ...
  var items = [
    ...
    { text: 33, icon: 'img/op_unhide.png', isEnabled: () => canHideUnhide() && hidden, fn: () => opHideUnhide(le, false) },
  ]
...
function onReady() {
  ...
  bindDragAndDropEvents($('[prevent-dnd]'), preventDragAndDrop)
  updateMarked()
  detectMediaFeatures()
...
function bindTitle(le, name) {
  le.find('#title').text(name).attr('title', name)
}
...
function opDelete(sel) {
  if (!sel) sel = getMarkedLeInGrid()
  if (!sel.length) sel = currentDir
  askToDelete(sel)
}
...
function showMediaViewer(allMedia, currI) {
  ...
  canList ? counter.show() : counter.hide()
  ...
  function fullscreenOn() {
    ...
    if (t.requestFullscreen) {
      ...
      doc.off('fullscreenchange').on('fullscreenchange', () => {

      })
    } else if (t.msRequestFullscreen) {
      t.msRequestFullscreen()
      fullscreenExit = document.msExitFullscreen
      doc.off('msfullscreenchange').on('msfullscreenchange', () => {
      ...
      doc.off('mozfullscreenchange').on('mozfullscreenchange', () => {
      ...
      doc.off('webkitfullscreenchange').on('webkitfullscreenchange', () => {
  ...
  function options() {
    ...
    var buttons = dlg.find('#buttons')
    {
      ...
      var e = $(`
        <div>${localizedStrings[41]}: 
          <input type="range" size="4" min="${ssdMin}" max="${ssdMax}">
          <span id="sec" style="min-width: 20px display: inline-block text-align: right"></span>
          ${localizedStrings[42]}
        </div>
      `)
      buttons.before(e)
